...
import { useState } from "react"; import Link from "next/link"; import { Shell } from "@/components/Shell"; import { Card, SectionLabel, Button, Pill } from "@/components/ui"; import { Strategy } from "@/lib/playbook";

type Candidate = { ticker: string; meta: Record<string, any>; };

type BriefSection = { label: string; value: string };

type Brief = { ticker: string; strategy: Strategy; sections: BriefSection[]; generated_at: string; };

export default function ScannerPage() { const [strategy, setStrategy] = useState("PEAD"); const [loading, setLoading] = useState(false); const [results, setResults] = useState<Candidate[] | null>(null); const [err, setErr] = useState<string | null>(null);

// Brief modal state const [briefOpen, setBriefOpen] = useState(false); const [briefTicker, setBriefTicker] = useState<string | null>(null); const [briefLoading, setBriefLoading] = useState(false); const [briefData, setBriefData] = useState<Brief | null>(null); const [briefErr, setBriefErr] = useState<string | null>(null);

async function runScan() { setErr(null); setResults(null); setLoading(true); try { const res = await fetch(/api/scan/${strategy.toLowerCase()}); const data = await res.json(); if (!res.ok) { setErr(data.error || "Scan failed"); } else { setResults(data.candidates || []); } } catch (e: any) { setErr(e.message); } finally { setLoading(false); } }

async function loadBrief(candidate: Candidate, force = false) { setBriefOpen(true); setBriefTicker(candidate.ticker); setBriefErr(null); if (!force && briefData?.ticker === candidate.ticker) return; // already loaded setBriefData(null); setBriefLoading(true); try { // Pass scanner-card meta along so the brief has snapshot context const metaParam = Buffer.from(JSON.stringify(candidate.meta)).toString( "base64" ); const res = await fetch( /api/grade/${candidate.ticker}?strategy=${strategy}&meta=${encodeURIComponent( metaParam )}${force ? &t=${Date.now()} : ""} ); const data = await res.json(); if (!res.ok) { setBriefErr(data.error || "Brief failed"); } else { setBriefData(data); } } catch (e: any) { setBriefErr(e.message); } finally { setBriefLoading(false); } }

function closeBrief() { setBriefOpen(false); }
